	...
	class Country
	{
		...
		function __construct()
		{
			...
			if (func_num_args() == 1)
			{
				...
				//query
				$query = 'select code, name from countries_ctg where code = ?';//the ? is a param
				//command
		...
		public static function getAll()
		{
			...
			$connection = MySQLConnection::getConnection();
			//query
			$query = 'select code, name from countries_ctg';
			//command
			$command = $connection->prepare($query);
			//execute
			$command->execute();
			...
			while ($command->fetch())
			{
				array_push($list, new Country($id, $name));
			}
			//close statement
			mysqli_stmt_close($command);
			//close connection
		...
		public static function getAllJson()
		{
			...
			return json_encode(array('countries' => $list));
		}
